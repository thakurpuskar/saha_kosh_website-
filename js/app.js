document.addEventListener("DOMContentLoaded", function () {
  ...
  function incrementKpiByLabel(labelSubstring, amount) {
    document.querySelectorAll(".kpi-label").forEach(function (label) {
      if (
        label.textContent
          .toLowerCase()
          .indexOf(labelSubstring.toLowerCase()) !== -1
      ) {
        ...
        if (valueEl) {
          ...
          if (!isNaN(n))
            valueEl.textContent = (n + amount).toLocaleString("en-US");
        }
      }
    });
  }
  document.querySelectorAll(".tabs").forEach(function (tabGroup) {
    tabGroup.querySelectorAll("a").forEach(function (tab) {
      tab.addEventListener("click", function (e) {
        ...
        tabGroup.querySelectorAll("a").forEach(function (t) {
  ...
  if (themeToggle) {
    themeToggle.addEventListener("click", function () {
      ...
      try {
        window.localStorage.setItem("sahakosh_theme", next);
      } catch (e) {}
    });
  }
  ...
  function renderMemberRow(m) {
    ...
    return (
      '<tr class="row-new">' +
      '<td style="color:var(--blue-700);font-weight:700;">' +
      escapeHtml(m.id) +
      "</td>" +
      '<td><div style="display:flex;align-items:center;gap:8px;"><span style="width:32px;height:32px;border-radius:12px;background:var(--blue-100b);display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:var(--blue-700);">' +
      initials +
      "</span><strong>" +
      escapeHtml(m.name) +
      "</strong></div></td>" +
      ...
      "</td>" +
      "<td><strong>" +
      escapeHtml(m.balance) +
      "</strong></td>" +
      '<td><span class="chip ' +
      escapeHtml(m.loanChip || "grey") +
      '">' +
      escapeHtml(m.loanStatus || "None") +
      "</span></td>" +
      "<td>" +
      escapeHtml(m.verification || "○ Pending") +
      "</td>" +
      '<td style="text-align:right;"><a href="member-profile.html?id=' +
      encodeURIComponent(m.id) +
      '" style="color:var(--blue-700);font-weight:700;font-size:12px;">View Details</a></td>' +
      "</tr>"
    );
  }
  ...
  function renderDepositRow(d) {
    return (
      '<tr class="row-new">' +
      "<td>" +
      escapeHtml(d.depositor) +
      "</td>" +
      "<td>" +
      escapeHtml(d.account) +
      "</td>" +
      "<td>Rs. " +
      escapeHtml(d.amountFmt) +
      "</td>" +
      "<td>" +
      escapeHtml(d.method) +
      "</td>" +
  ...
  function renderEventCard(e) {
    return (
      '<div class="card row-new" style="overflow:hidden;">' +
    ...
    var stockChip =
      p.stock === "Low Stock"
        ...
        : p.stock === "Out of Stock"
          ...
          : "green";
    return (
      '<div class="card row-new" style="padding:17px;">' +
      '<div style="height:120px;background:var(--blue-50);border-radius:4px;margin-bottom:12px;display:flex;align-items:center;justify-content:center;font-size:32px;">' +
      escapeHtml(p.icon || "📦") +
      "</div>" +
      ...
      escapeHtml(p.seller) +
      "</div>" +
      '<div style="display:flex;justify-content:space-between;margin-top:8px;"><strong>Rs. ' +
      escapeHtml(p.priceFmt) +
  ...
  function renderCampaignCard(c) {
    ...
    return (
      '<div class="card panel row-new">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><h3 class="panel-h3">' +
      escapeHtml(c.name) +
      '</h3><span class="chip ' +
      urgencyChip +
      '">' +
      ...
      "</span></div>" +
      '<div style="font-size:24px;font-weight:700;color:var(--blue-700);margin-bottom:4px;">Rs. 0 <span style="font-size:13px;font-weight:400;color:var(--text-muted);">of Rs. ' +
      escapeHtml(c.goalFmt) +
      "</span></div>" +
      '<div class="progress-track" style="margin-bottom:8px;"><div class="progress-fill" style="width:0%;"></div></div>' +
      '<div style="font-size:13px;color:var(--text-muted);">0 donors · ' +
      escapeHtml(String(c.daysLeft || 30)) +
      " days left</div>" +
      "</div>"
    );
  }
  function renderOpportunityCard(o) {
    return (
      ...
      escapeHtml(o.details) +
      "</div>" +
      '<span class="chip green">' +
      (o.extra ? escapeHtml(o.extra) : "New") +
      "</span>" +
      "</div>"
    );
  }
  ...
  if (SK) {
    if (/Member Records/i.test(pageTitle)) {
      ...
      if (mTbody && custom.length)
        prependHtml(mTbody, custom.map(renderMemberRow));
    }
    if (/Transaction (Management|Ledger)/i.test(pageTitle)) {
      ...
      if (tTbody && txns.length)
        prependHtml(tTbody, txns.map(renderTransactionRow));
    }
    if (/Deposit Management/i.test(pageTitle)) {
      ...
      if (dTbody && deps.length)
        prependHtml(dTbody, deps.map(renderDepositRow));
    }
    if (/Community/i.test(pageTitle)) {
      var evGrid = document.querySelector(".grid-3");
      var evs = SK.getEntries("events");
      if (evGrid && evs.length) {
        prependHtml(evGrid, evs.map(renderEventCard));
        incrementKpiByLabel("Upcoming Events", evs.length);
      }
    }
    if (/Marketplace/i.test(pageTitle)) {
      var prGrid = document.querySelector(".grid-3");
      var prods = SK.getEntries("products");
      if (prGrid && prods.length) {
        prependHtml(prGrid, prods.map(renderProductCard));
        incrementKpiByLabel("Listed Products", prods.length);
      }
    }
    if (/Donations/i.test(pageTitle)) {
      var caGrid = document.querySelector(".grid-2");
      var camps = SK.getEntries("campaigns");
      if (caGrid && camps.length) {
        prependHtml(caGrid, camps.map(renderCampaignCard));
        incrementKpiByLabel("Active Campaigns", camps.length);
      }
    }
    if (/Opportunities/i.test(pageTitle)) {
      var opGrid = document.querySelector(".grid-3");
      var opps = SK.getEntries("opportunities");
      if (opGrid && opps.length) {
        prependHtml(opGrid, opps.map(renderOpportunityCard));
        incrementKpiByLabel("Open Opportunities", opps.length);
      }
    }
  }
  ...
  var MODAL_DEFS = [
    {
      ...
      subtitle: "Register a new member and open their savings account.",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "select",
          options: ["Regular Savings", "Fixed Deposit", "Group / Cooperative"],
        },
        {
          name: "deposit",
          label: "Initial Deposit (NPR)",
          type: "number",
          placeholder: "0.00",
        },
        {
    ...
    },
    {
      ...
      triggers: ["new transaction"],
      title: "New Transaction",
      subtitle: "Record a deposit, withdrawal, or EMI payment.",
      ...
      fields: [
        {
          ...
          required: true,
          full: true,
          placeholder: "e.g. Sanjeev Shrestha — SB-44201",
        },
        {
          name: "type",
        ...
        },
        {
          ...
          label: "Method",
          type: "select",
          options: ["Cash", "Bank Transfer", "Mobile Wallet"],
        },
        {
          name: "note",
    ...
    {
      name: "new-deposit",
      triggers: ["new deposit", "record deposit"],
      title: "New Deposit",
      subtitle: "Record a savings deposit for a member or group.",
      ...
      fields: [
        {
          ...
          required: true,
          full: true,
          placeholder: "e.g. Sanjeev Shrestha or Agriculture Group B",
        },
        {
          name: "account",
          label: "Account No.",
    ...
    },
    {
      ...
      triggers: ["new event"],
      title: "New Event",
      subtitle: "Create a community event or workshop.",
      ...
      fields: [
        ...
        },
        {
          ...
          type: "text",
          required: true,
          placeholder: "e.g. Itahari Community Hall",
        },
        {
          name: "category",
          label: "Category",
          type: "select",
          options: ["General", "Training", "Social"],
        },
        {
          name: "rsvps",
    ...
    {
      name: "list-product",
      triggers: ["list product"],
      title: "List New Product",
      subtitle: "Add a product to the community marketplace.",
      ...
      fields: [
        {
          ...
          required: true,
          full: true,
          placeholder: "e.g. Organic Rice — 25kg",
        },
        {
          name: "seller",
          label: "Seller / Group",
          type: "text",
          required: true,
          placeholder: "e.g. Agriculture Group B",
        },
        {
          name: "price",
          label: "Price (NPR)",
    ...
    {
      name: "new-campaign",
      triggers: ["new campaign"],
      title: "New Campaign",
      subtitle: "Launch a fundraising or donation campaign.",
    ...
    },
    {
      ...
      triggers: ["post opportunity"],
      title: "Post Opportunity",
      subtitle: "Share a job, training, or grant with members.",
      ...
      fields: [
        {
          ...
          type: "select",
          required: true,
          options: ["Job Listing", "Skill Training", "Micro-grant"],
        },
        {
          name: "title",
          label: "Title",
          ...
          required: true,
          full: true,
          placeholder: "e.g. Agri-supply Coordinator",
        },
        {
          name: "details",
          label: "Details",
          ...
          required: true,
          full: true,
          placeholder: "e.g. Itahari Branch · Full-time",
        },
        {
          name: "extra",
          label: "Extra Info",
          type: "text",
          placeholder: "e.g. 12 applicants / Up to Rs. 50,000",
        },
      ],
    },
  ];
  ...
  modalOverlay.innerHTML = MODAL_DEFS.map(function (def) {
    ...
    return (
      ...
      def.name +
      '" novalidate><div class="form-grid">' +
      fieldsHtml +
      "</div>" +
  ...
  function closeModal() {
    ...
    modalOverlay.querySelectorAll(".modal").forEach(function (m) {
      m.classList.remove("is-active", "is-shown");
    });
    modalOverlay.querySelectorAll("form").forEach(function (f) {
      f.reset();
    });
  ...
  document
    .querySelectorAll(
      "button, a.btn, a.btn-primary, a.btn-outline, a.btn-ghost",
    )
  ...
  var onSubmitHandlers = {
    ...
    "new-transaction": function (fd) {
      ...
      var t = SK.addEntry("transactions", {
        ref: "TXN-" + Math.floor(50000 + Math.random() * 9000),
        member: fd.get("member") || "Walk-in",
        type: fd.get("type") || "Transaction",
        amountFmt: amount.toLocaleString("en-US", { minimumFractionDigits: 2 }),
    ...
    "new-deposit": function (fd) {
      if (!SK) return;
      var amount = parseFloat(fd.get("amount") || "0") || 0;
      var d = SK.addEntry("deposits", {
        depositor: fd.get("depositor") || "Unnamed",
    ...
    "new-event": function (fd) {
      ...
      var gradients = {
        General: "linear-gradient(135deg,#003d9b,#0865f5)",
        Training: "linear-gradient(135deg,#006c49,#6cf8bb)",
        Social: "linear-gradient(135deg,#603b00,#ffc988)",
      };
      var e = SK.addEntry("events", {
        title: fd.get("title") || "New Event",
        date: fd.get("date") || "TBA",
        location: fd.get("location") || "—",
        category: category,
        gradient: gradients[category] || gradients.General,
        rsvps: fd.get("rsvps") || 0,
      });
      if (/Community/i.test(pageTitle)) {
        var grid = document.querySelector(".grid-3");
        if (grid) {
          prependHtml(grid, [renderEventCard(e)]);
          incrementKpiByLabel("Upcoming Events", 1);
        }
      }
    ...
    "list-product": function (fd) {
      ...
      var p = SK.addEntry("products", {
        name: fd.get("name") || "New Product",
        seller: fd.get("seller") || "—",
        priceFmt: price.toLocaleString("en-US"),
        stock: fd.get("stock") || "In Stock",
        icon: fd.get("icon") || "📦",
      });
      if (/Marketplace/i.test(pageTitle)) {
        var grid = document.querySelector(".grid-3");
        if (grid) {
          prependHtml(grid, [renderProductCard(p)]);
          incrementKpiByLabel("Listed Products", 1);
        }
      }
    ...
    "new-campaign": function (fd) {
      ...
      var c = SK.addEntry("campaigns", {
        name: fd.get("name") || "New Campaign",
        goalFmt: goal.toLocaleString("en-US"),
        urgency: fd.get("urgency") || "Ongoing",
        daysLeft: fd.get("daysLeft") || 30,
      });
      if (/Donations/i.test(pageTitle)) {
        var grid = document.querySelector(".grid-2");
        if (grid) {
          prependHtml(grid, [renderCampaignCard(c)]);
          incrementKpiByLabel("Active Campaigns", 1);
        }
      }
    ...
    "post-opportunity": function (fd) {
      if (!SK) return;
      var o = SK.addEntry("opportunities", {
        type: fd.get("type") || "Job Listing",
        title: fd.get("title") || "New Opportunity",
      ...
      if (/Opportunities/i.test(pageTitle)) {
        var grid = document.querySelector(".grid-3");
        if (grid) {
          prependHtml(grid, [renderOpportunityCard(o)]);
          incrementKpiByLabel("Open Opportunities", 1);
        }
      }
  ...
  if (loanFilterBtns.length) {
    loanFilterBtns.forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (showingEl)
          showingEl.textContent = "Showing " + visible + " of 482 loans";
      });
    });
  }
  var REPORT_CONTENT = {
    "Monthly Financial Summary": [
      "Total Savings Pool: Rs. 4.2M (+ Rs. 450K month-over-month)",
      "Total Loans Disbursed: रू 12.4M",
      "Total Collections This Month: Rs. 1.8M",
      "Net Interest Income: Rs. 168K year-to-date",
    ],
    "Branch Performance Report": [
      "Itahari Branch — Rs. 1.4M savings, 96% collection rate",
      "Kathmandu Branch — Rs. 1.1M savings, 91% collection rate",
      "Lalitpur Branch — Rs. 900K savings, 94% collection rate",
      "Top performer this quarter: Itahari Branch",
    ],
    "Regulatory Compliance Report": [
      "Audit Score: 98 / 100",
      "Compliance Checks Passed: 142 this month",
      "Flagged Transactions Under Review: 3",
      "Last Full Audit: Aug 1, 2026",
    ],
  };
  var reportButtons = Array.prototype.filter.call(
    document.querySelectorAll(".content .btn"),
    function (b) {
  ...
  if (reportButtons.length) {
    ...
    function openReport(name) {
      ...
      reportOverlay.querySelector("#reportModalDate").textContent =
        "Generated " +
      ...
      var lines = REPORT_CONTENT[name] || [
        "Report data is being compiled for this section.",
      ];
      reportOverlay.querySelector("#reportModalBody").innerHTML =
        '<ul style="margin:0;padding-left:20px;display:flex;flex-direction:column;gap:10px;font-size:14px;">' +
        lines
          .map(function (l) {
    ...
    reportOverlay.querySelectorAll("[data-report-close]").forEach(function (b) {
  ...
  document.querySelectorAll(".content .btn").forEach(function (btn) {
    if (
      ...
      btn.hasAttribute("data-report-trigger") ||
      btn.classList.contains("loan-filter-btn") ||
      btn.closest(".tabs")
    )
      return;
    btn.addEventListener("click", function () {
